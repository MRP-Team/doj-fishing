...
RegisterNetEvent('fishing:server:catch', function(maxLuck)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local luck = math.random(1, maxLuck)
    ...
    if itemFound then
        for i = 1, itemCount, 1 do
            if luck == 100 then
						...
						elseif luck >= 20 then
							Player.Functions.AddItem('bottle', 1, nil, {["quality"] = 100})
							TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['bottle'], "add", 1)
							TriggerClientEvent('QBCore:Notify', src, "You caught an empty bottle!", "primary")
						else
							Player.Functions.AddItem('can', 1, nil, {["quality"] = 100})
							TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['can'], "add", 1)
							TriggerClientEvent('QBCore:Notify', src, "You caught a empty can!", "primary")
            end
            Citizen.Wait(500)
        end
    end
